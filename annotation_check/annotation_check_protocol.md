...
7. Run `script_reformat_proteome` to transform the text file (.txt) into a FASTA sequence file (.faa) of amino acid sequences, which the format OrthoFinder requires for input files. This script will save the .faa files into the directory in this repository `proteome_2`.
